# API Overview
InkTail usses Next.js with the new APP router to create a RESTful API. The API is organized into routes, each of which is a directory in 
...
a higher order function `withAuth` is used to wrap each route handler to catch any errors that occur during the request/response
authorization cycle.
...
1. Create a new directory in `src/app/api/<name-of-route>/route.ts`. For example, if you are creating an endpoint called 
   `users`, create a new directory called `users` in `src/app/api` then create a `route.ts` in that folder. The file structure would look like: `src/app/api/users/route.ts`
